...
- (UIView *)topNavationBar {
  if ([self.delegate
          respondsToSelector:@selector(customNavigationBarOfImageViewer:)]) {
...
- (void)prepareToDismiss {
  ...
  if ([self.delegate respondsToSelector:@selector(imageViewer:
                                            willDismissWithSelectedView:)]) {
...
//          currentView.transform = state.transform;
      ...
      completion:^(BOOL finished) {
          ...
          if ([self.delegate
                  respondsToSelector:@selector(imageViewer:
